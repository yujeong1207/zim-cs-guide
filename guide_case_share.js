/* =========================================================================
   📚 케이스 공유 (수출 CS) - 팔로우업 보드와 달리, "건을 처리하는 중" 추적용이 아니라
   이미 겪은 사례를 길게 써서 두고두고 참고하려고 모아두는 게시판이에요.
   위임장/팔로우업보드와 완전히 같은 방식(Firestore)이라 팀원 누구나
   바로 작성·수정·삭제할 수 있고, 다른 사람 화면에도 실시간으로 반영돼요.
   ========================================================================= */
const CASE_SHARE_COLLECTION = "case_share"; // Firestore 컬렉션 이름

let CASE_SHARE_LIST = [];
let caseShareUnsubscribe = null;
let caseShareDraft = null; // 지금 편집중인 케이스 (없으면 새 케이스)
let caseShareExpanded = new Set(); // 본문을 펼쳐서 보고 있는 카드 id 모음

const CASE_SHARE_CATEGORIES = ["스케줄", "COD", "정산/비용", "클레임", "통관/서류", "기타"];

/* Firestore에서 케이스 전체 목록을 가져온다 (최초 1회, onSnapshot 실패시 폴백용) */
async function fetchCaseShareListFromServer() {
  try {
    await window.fbReady;
    const snapshot = await window.fbDb.collection(CASE_SHARE_COLLECTION).get();
    return snapshot.docs.map((doc) => caseShareDocToEntry(doc));
  } catch (err) {
...
function caseShareDocToEntry(doc) {
  const d = doc.data();
  return {
    id: doc.id,
    caseDate: d.caseDate || "",
...
async function submitCaseShareToServer(entry) {
  try {
    await window.fbReady;
    const nowIso = new Date().toISOString();
    const docRef = await window.fbDb.collection(CASE_SHARE_COLLECTION).add({
...
async function updateCaseShareOnServer(entry) {
  try {
    await window.fbReady;
    await window.fbDb.collection(CASE_SHARE_COLLECTION).doc(entry.id).update({
      caseDate: entry.caseDate || "",
      title: entry.title || "",
      category: entry.category || "기타",
      body: entry.body || "",
...
async function deleteCaseShareFromServer(id) {
  try {
    await window.fbReady;
    await window.fbDb.collection(CASE_SHARE_COLLECTION).doc(id).delete();
    return { ok: true };
  } catch (err) {
...
async function toggleCaseSharePinned(id) {
  const item = CASE_SHARE_LIST.find((c) => c.id === id);
  if (!item) return;
  try {
    await window.fbReady;
    await window.fbDb.collection(CASE_SHARE_COLLECTION).doc(id).update({ pinned: !item.pinned });
  } catch (err) {
...
async function loadCaseShareTab(forceRefresh) {
  const wrap = document.getElementById("caseShareListWrap");
  if (liveSubscribed.caseShare && !forceRefresh) { renderCaseShareList(); return; }
  if (forceRefresh && caseShareUnsubscribe) { caseShareUnsubscribe(); caseShareUnsubscribe = null; liveSubscribed.caseShare = false; }
  if (wrap && !CASE_SHARE_LIST.length) wrap.innerHTML = '<div class="empty-state">⏳ 최신 케이스 목록을 불러오는 중이에요...</div>';
  await window.fbReady;
  caseShareUnsubscribe = window.fbDb.collection(CASE_SHARE_COLLECTION).onSnapshot(
    (snapshot) => {
      CASE_SHARE_LIST = snapshot.docs.map((doc) => caseShareDocToEntry(doc));
      renderCaseShareList();
    },
    (err) => {
      console.error("케이스 공유 실시간 구독 실패:", err);
      if (wrap && !CASE_SHARE_LIST.length) wrap.innerHTML = '<div class="empty-state">⚠️ 최신 목록을 불러오지 못했어요 (네트워크 문제일 수 있어요). <button class="btn secondary-btn" style="padding:2px 10px;font-size:12px;margin-left:6px;" onclick="loadCaseShareTab(true)">다시 시도</button></div>';
    }
  );
  liveSubscribed.caseShare = true;
  liveTabUnsubscribers.caseShare = () => { if (caseShareUnsubscribe) { caseShareUnsubscribe(); caseShareUnsubscribe = null; liveSubscribed.caseShare = false; } };
}

function caseShareFilteredList() {
  const qEl = document.getElementById("caseShareFilter");
  const q = (qEl ? qEl.value : "").trim().toLowerCase();
  const catEl = document.getElementById("caseShareCategoryFilter");
  const catFilter = catEl ? catEl.value : "__all";

  let list = CASE_SHARE_LIST.slice();
...
function caseShareCategoryBadgeStyle(category) {
  const map = {
...
function renderCaseShareList() {
  ...
  list.forEach((c) => wrap.appendChild(buildCaseShareCard(c)));
}

function buildCaseShareCard(c) {
  const isExpanded = caseShareExpanded.has(c.id);
  ...
  catBadge.style.cssText = "font-size:11.5px;font-weight:600;padding:2px 8px;border-radius:6px;" + caseShareCategoryBadgeStyle(c.category);
  ...
  pinBtn.textContent = "📌";
  pinBtn.onclick = (e) => { e.stopPropagation(); toggleCaseSharePinned(c.id); };
  const editBtn = document.createElement("button");
  editBtn.className = "btn secondary-btn";
  editBtn.style.cssText = "padding:4px 10px;font-size:12px;";
  editBtn.textContent = "✏️ 수정";
  editBtn.onclick = (e) => { e.stopPropagation(); openCaseShareEditor(c.id); };
  ...
  toggleBtn.onclick = (e) => {
    e.stopPropagation();
    if (isExpanded) caseShareExpanded.delete(c.id); else caseShareExpanded.add(c.id);
    renderCaseShareList();
  };
...
/* ---- 등록/수정 모달 ---- */
function openCaseShareEditor(existingId) {
  caseShareDraft = existingId ? Object.assign({}, CASE_SHARE_LIST.find((c) => c.id === existingId)) : {
    id: null,
    caseDate: new Date().toISOString().slice(0, 10),
    title: "", category: "기타", body: "", author: "", pinned: false,
  };
  document.getElementById("caseShareEditTitle").textContent = existingId ? "✏️ 케이스 수정" : "📚 케이스 작성하기";
  document.getElementById("caseShareEditOverlay").style.display = "flex";
  renderCaseShareEditorBody();
}

function closeCaseShareEditor() {
  document.getElementById("caseShareEditOverlay").style.display = "none";
  caseShareDraft = null;
}

function renderCaseShareEditorBody() {
  const body = document.getElementById("caseShareEditBody");
  body.innerHTML = "";
  const d = caseShareDraft;
  ...
  const categorySel = makeFollowupSelect(CASE_SHARE_CATEGORIES, d.category);
  ...
  saveBtn.onclick = async () => {
    ...
    const result = entry.id ? await updateCaseShareOnServer(entry) : await submitCaseShareToServer(entry);
    saveBtn.disabled = false;
    saveBtn.textContent = "💾 저장하기";
    if (!result.ok) { alert("저장에 실패했어요: " + (result.error || "알 수 없는 오류")); return; }
    closeCaseShareEditor();
  };
  ...
  deleteBtn.onclick = async () => {
    ...
    const result = await deleteCaseShareFromServer(d.id);
  ...
  cancelBtn.onclick = () => closeCaseShareEditor();
